...
const EmployeeManagement: React.FC = () => {
  ...
  // Optimize O(N*M) lookup to O(1) Map
  const statsMap = useMemo(() => {
    ...
    // Process assignments in one pass
    ...
    // Process certificates in one pass
  ...
  const totalPages = useMemo(() => Math.ceil(filteredEmployees.length / PAGE_SIZE), [filteredEmployees.length]);
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedEmployees.map((employee) => {
                    const stats = statsMap.get(employee.id) || emptyStats;
                    return (
