...
const EmployeeManagement: React.FC = () => {
  ...
  // ⚡ Bolt Optimization: Memoize the base employees array
  const employees = useMemo(() => users.filter(u => u.role === 'employee'), [users]);

  // ⚡ Bolt Optimization: Replace O(N*M) derived calculations with an O(N+M) single-pass Map
  const employeeStatsMap = useMemo(() => {
    ...
    assignments.forEach(assignment => {
      const stats = map.get(assignment.userId);
      if (stats) {
    ...
    certificates.forEach(cert => {
      const stats = map.get(cert.userId);
      if (stats) stats.certificates++;
    });
  ...
  // ⚡ Bolt Optimization: Memoize filtered list to avoid re-filtering when unrelated state changes
  ...
  // ⚡ Bolt Optimization: Memoize aggregate calculations
