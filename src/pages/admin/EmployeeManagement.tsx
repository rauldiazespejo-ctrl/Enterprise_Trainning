...
const EmployeeManagement: React.FC = () => {
  ...
  // Optimization: Pre-calculate employee stats using a single pass to map to O(1) lookups
  // avoiding O(N*M) time complexity during renders where employeeStats() previously
  // iterated assignments and certificates for each user inside map/reduce loops.
  const statsMap = useMemo(() => {
    const map = new Map<string, { completed: number; inProgress: number; certificates: number }>();

    // Initialize map for all employees to handle default values
    employees.forEach(emp => {
      map.set(emp.id, { completed: 0, inProgress: 0, certificates: 0 });
    });

    // Single pass on assignments
  ...
  const totalCompleted = employees.reduce((sum, e) => sum + (statsMap.get(e.id)?.completed || 0), 0);
  const totalInTraining = employees.filter(e => (statsMap.get(e.id)?.inProgress || 0) > 0).length;
