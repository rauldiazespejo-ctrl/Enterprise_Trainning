...
const emptyStats = { completed: 0, inProgress: 0, certificates: 0 };
...
const EmployeeManagement: React.FC = () => {
  ...
  const employeeStatsMap = useMemo(() => {
    ...
    // Bulk count certificates
    certificates.forEach(c => {
      const userStats = stats.get(c.userId);
      if (userStats) {
  ...
  const totalCompleted = useMemo(() => employees.reduce((sum, e) => sum + (employeeStatsMap.get(e.id) || emptyStats).completed, 0), [employees, employeeStatsMap]);
  const totalInTraining = useMemo(() => employees.filter(e => (employeeStatsMap.get(e.id) || emptyStats).inProgress > 0).length, [employees, employeeStatsMap]);
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedEmployees.map((employee) => {
                    const stats = employeeStatsMap.get(employee.id) || emptyStats;
                    return (
