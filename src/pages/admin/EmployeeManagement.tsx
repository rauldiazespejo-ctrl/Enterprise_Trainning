...
const EmployeeManagement: React.FC = () => {
  ...
  // Memoized stats calculation in O(N+M) pass
  ...
  const totalCompleted = employees.reduce((sum, e) => sum + (statsMap.get(e.id) || DEFAULT_STATS).completed, 0);
  const totalInTraining = employees.filter(e => (statsMap.get(e.id) || DEFAULT_STATS).inProgress > 0).length;
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedEmployees.map((employee) => {
                    const stats = statsMap.get(employee.id) || DEFAULT_STATS;
                    return (
