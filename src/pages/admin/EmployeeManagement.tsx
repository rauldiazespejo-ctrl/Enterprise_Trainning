...
const EmployeeManagement: React.FC = () => {
  ...
  const fallbackStats = { completed: 0, inProgress: 0, certificates: 0 };
  ...
  const totalCompleted = employees.reduce((sum, e) => sum + (statsMap.get(e.id) || fallbackStats).completed, 0);
  const totalInTraining = employees.filter(e => (statsMap.get(e.id) || fallbackStats).inProgress > 0).length;
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedEmployees.map((employee) => {
                    const stats = statsMap.get(employee.id) || fallbackStats;
                    return (
