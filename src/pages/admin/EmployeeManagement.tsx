...
const EmployeeManagement: React.FC = () => {
  ...
  // Optimize lookups by building a lookup map inside useMemo to prevent recalculation on every render
  const statsMap = useMemo(() => {
    const map = new Map<string, { completed: number, inProgress: number, certificates: number }>();

    // Pre-calculate certificates count per user to avoid O(N*M) filter inside the loop
    const certCountMap = new Map<string, number>();
    certificates.forEach(c => {
      certCountMap.set(c.userId, (certCountMap.get(c.userId) || 0) + 1);
    });

    employees.forEach(emp => {
      // Use getUserAssignments to maintain encapsulation and underlying business logic
      ...
      map.set(emp.id, {
        ...
        certificates: certCountMap.get(emp.id) || 0
      });
    });

    return map;
  }, [employees, certificates, getUserAssignments]);
  ...
  const totalCompleted = useMemo(() =>
    employees.reduce((sum, e) => sum + (employeeStats(e.id)?.completed || 0), 0),
  [employees, employeeStats]);

  const totalInTraining = useMemo(() =>
    employees.filter(e => (employeeStats(e.id)?.inProgress || 0) > 0).length,
  [employees, employeeStats]);
