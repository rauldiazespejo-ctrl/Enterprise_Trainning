...
const AssignmentManagement: React.FC = () => {
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize filtered arrays to prevent recalculation on every render
  ...
  // ⚡ BOLT OPTIMIZATION: Convert O(N*M) nested array lookups into O(N+M) Map lookups, and memoize
  // to avoid expensive re-calculations on every keystroke in the search term.
  ...
  // ⚡ BOLT OPTIMIZATION: Memoize derived search/filter results
  const filtered = useMemo(() => {
    const searchLower = searchTerm.toLowerCase();
    return enrichedAssignments.filter(a => {
      if (searchLower && !a.employeeName.toLowerCase().includes(searchLower) && !a.courseTitle.toLowerCase().includes(searchLower)) return false;
